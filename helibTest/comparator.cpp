...
void Comparator::create_poly()
{
	...
	if(m_type == UNI)
	{
		...
		{ 
			...
			// f_i is coefficient of Z^i in polynomial P(Z)
			...
			m_univar_less_poly += ZZX(INIT_MONO, (indx-1) >> 1, rep(coef));		// indx=5 means 2; indx=7 means 3
		}
...
// x has ciphertext for (n1-n2); ret and ctxt_p_1 are Ciphertext for less-than and equality respectively
...
void Comparator::less_than_bivar(Ctxt& ctxt_res, const Ctxt& ctxt_x, const Ctxt& ctxt_y) const
{
  ...
  if(p > 7)
  {
   ...
   cout<<"Printing result after"<<endl;
   cout<<"################################################"<<endl;
...
void Comparator::compare(Ctxt& ctxt_res, const Ctxt& ctxt_x, const Ctxt& ctxt_y) const
{
	...
	else // univariate circuit
	{
		...
		for (long iCoef = 0; iCoef < 1; iCoef++)
		{
			Ctxt ctxt_tmp = Ctxt(ctxt_z.getPubKey());
		...
		// cout<<"Univariate comparison result"<<endl;
		// print_decrypted(ctxt_less_p[0]);
    	// cout << endl;

		// cout<<"Univariate finished"<<endl;	
	}

	//cout << "Compare digits" << endl;
	Ctxt ctxt_less = ctxt_less_p[0];
	// Ctxt ctxt_eq = ctxt_eq_p[0];
	...
	ctxt_res = ctxt_less;
	// cout<<"Returning from compare()"<<endl;
	return;
...
void Comparator::test_compare(long runs=1) const
{
//   //reset timers
//   setTimersOn();
  
  cout<<"Input number of pairs of comparisons needed"<<endl;
  int c; cin>>c; int* f_pair; int* s_pair;
  f_pair=(int*)malloc(c*sizeof(int)); s_pair = (int*)malloc(c*sizeof(int));

  cout<<"Enter all first numbers amongst all pairs"<<endl;
  for(int kk=0;kk<c;kk++)
	cin>>f_pair[kk];

cout<<"Enter all second numbers amongst all pairs"<<endl;
  for(int kk=0;kk<c;kk++)
	cin>>s_pair[kk];
  ...
  vector<ZZX> expected_result(nslots);
	...
	for(int kk=0;kk<c;kk++)
	{
		if(f_pair[kk]<s_pair[kk])
			expected_result[kk] = ZZX(INIT_MONO, 0, 1);
		else
			expected_result[kk] = ZZX(INIT_MONO, 0, 0);
		cout<<"expected_result["<<kk<<"]= "<<expected_result[kk]<<endl;
	}

	// if (n1<n2)
	// {
	// 	expected_result[0] = ZZX(INIT_MONO, 0, 1);
	// }
	// else
	// {
	// 	expected_result[0] = ZZX(INIT_MONO, 0, 0);
	// }

	cout<<"Expected: "<<endl;
	for(int kk=0;kk<c;kk++)
		printZZX(cout,expected_result[kk], ord_p);
	...
	for(int kk=0;kk<c;kk++)
	{
		pol_x[kk]=ZZX(INIT_MONO, 0, f_pair[kk]);
		pol_y[kk]=ZZX(INIT_MONO, 0, s_pair[kk]);// produce a ZZX (degree 0 polynomial)	
	}

	// pol_x[0]=ZZX(INIT_MONO, 0, n1);		// Constant term in polynomial is the number as it is <p
	// pol_y[0]=ZZX(INIT_MONO, 0, n2);		

	// cout<<"pol_x[0]: "<<pol_x[0]<<"; pol_y[0]: "<<pol_y[0]<<endl;
	// cout<<"pol_x: "<<endl;
	// for(int kk=0;kk<c;kk++)
	// {
	// 	cout<<pol_x[kk]<<" ";	
...
void Comparator::test_array_min(int input_len, long depth, long runs) const
{
  ...
  for (int run = 0; run < runs; run++)
  {
	...
	*/
    ...
    cout << "Result of min: ";
    printZZX(cout, decrypted[0], ord_p);
    cout << endl;
    cout << "Result of max: ";
    printZZX(cout, decrypted[nslots-1], ord_p);
    cout << endl;*/
    printNamedTimer(cout, "Extraction");
    printNamedTimer(cout, "ComparisonCircuitBivar");
    printNamedTimer(cout, "ComparisonCircuitUnivar");
